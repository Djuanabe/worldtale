...
2. **SQL Editor** を開き、`supabase/migrations/` の SQL を**番号順に**貼り付けて実行:
   ...
   - `0006_moderation.sql`（報告対応・警告のための管理者フラグ／警告テーブル）
   （続けて貼り付けて一度に実行してもよい。既存環境には未適用の番号だけ追加実行すればよい）
...
- パスワードは PBKDF2-SHA256 を複数段チェインして保存（実効30万回）。ログイン成功時に
  旧い保存形式は現行パラメータへ自動アップグレードされる。

### モデレーション（報告対応・警告）

報告された投稿はアプリ内の管理画面 **`/admin`** で確認・対応できる（要・管理者権限）。

1. 自分のアカウントを管理者にする（Supabase の SQL Editor で一度だけ実行）:
   ```sql
   update public.users set is_admin = true where public_id = '<自分のログインID>';
   ```
2. アプリを再読み込みするとヘッダーに「管理」リンクが出る。`/admin` で:
   - 未対応／対応済み／却下／すべて で報告を絞り込み
   - **非表示にする／再表示** … `is_hidden` の切り替え（推奨・可逆）
   - **削除する** … 投稿を完全削除（添付写真も Storage ごと削除・不可逆）
   - **作者に警告** … 本人がログイン時にアプリ内バナーで見る警告を送る
   - **対応済み／却下** … 報告のステータス更新
3. Supabase ダッシュボードで対象行の `is_hidden` を直接 `true` にしても非表示化できる。
