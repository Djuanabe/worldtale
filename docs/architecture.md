...
- **シーン**: ヘッダー下いっぱいに、消失点へ延びる道（遠近感のある台形）、両脇の野原、空を
  CSS/インラインSVGで描く。外部アセット不使用。見た目はデザインガイドの**レトロRPGフィールド調**
  （ドット調の空・草原・土の道。ふきだしはメッセージウィンドウ）。
- **アバター**: シンプルなヒト型シルエット（インラインSVG）。その都道府県（+年フィルタ）の
  物語からランダムに選ばれた1件を1体が担う。同時に3〜8体が道の上のランダムな奥行きに立ち、
  ゆっくり左右にうろうろ+上下に揺れる。一定時間（15〜40秒のランダム）でフェードアウトし、
  別のランダムな物語を持つ新しいヒトがフェードインする（見え隠れ）。
...
- ドット感: `image-rendering: pixelated` を画像に適用。角丸・ぼかし影は使わない。
  境界は 2〜4px のくっきりした枠線
- アバター/スプライト: ドット絵調のヒト型スプライト（SVG の rect グリッドで自前描画。
  8×12 グリッド程度の簡素なドット人間、歩行は2フレーム交互）
