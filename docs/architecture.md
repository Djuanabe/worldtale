...
- 操作は**2段階（地方ズーム → 都道府県選択）**:
  1. 全国図では**地方単位**（北海道/東北/関東/中部/近畿/中国/四国/九州・沖縄）で
     hover ハイライト+地方名・合計件数を表示。クリックでその地方へ CSS transform ズーム
  2. 地方ズーム中は**都道府県単位**で hover（県名+件数）、クリックで `/p/:pref` へ。
     「◀ 日本全体」ボタンで全国図へ戻る。ズーム中はカプセル入口を隠す
  - ズームは canvas の再描画ではなく scale+translate（pixelated なのでドットが太るだけで自然）。
    ポインタ→セル解決は getBoundingClientRect が transform を反映するため式は共通
  - アクセシビリティ用に都道府県セレクト（プルダウン）も併設（直接遷移）
